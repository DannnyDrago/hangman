...
public class GameRepository {
    ...
    public List<HangmanGame> getTop10RecentGames() {
        String query = "SELECT * FROM HANGMAN_GAME ORDER BY STARTED_ON_DATE DESC LIMIT 10";
        return jdbcTemplate.query(query, (rs, rowNum) -> {
            HangmanGame hangmanGame = new HangmanGame();
            hangmanGame.setId(rs.getLong("ID"));
            hangmanGame.setCurrentWord(rs.getString("CURRENT_WORD"));
            hangmanGame.setWrongGuesses(rs.getInt("WRONG_GUESSES"));
            hangmanGame.setStartedOnDate(rs.getObject("STARTED_ON_DATE", LocalDateTime.class));
            return hangmanGame;
        });
    }
